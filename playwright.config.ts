...
export default defineConfig({
    testDir: "./tests",
    /* Don't run tests in files in parallel because database operations would conflict. */
    fullyParallel: false,
    /* Fail the build on CI if you accidentally left test.only in the source code. */
    forbidOnly: !!process.env.CI,
    /* Retry on CI only */
    retries: process.env.CI ? 2 : 0,
    /* Opt out of parallel tests because database operations would conflict. */
    workers: 1,
    ...
    projects: [
        {
            ...
            use: {
                ...
                launchOptions: {
                    // Chromium issue: https://github.com/microsoft/playwright/issues/36395#issuecomment-2999532212
                    args: [
